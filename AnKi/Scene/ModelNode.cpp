...
void ModelNode::draw(RenderQueueDrawContext& ctx, ConstWeakArray<void*> userData, U32 modelPatchIdx) const
{
	...
	if(ANKI_LIKELY(!ctx.m_debugDraw))
	{
		...
		for(VertexStreamId streamId : EnumIterable<VertexStreamId>())
		{
			if(modelInf.m_vertexBufferOffsets[streamId] == MAX_PTR_SIZE)
			{
				continue;
			}
			...
			const Format fmt = REGULAR_VERTEX_STREAM_FORMATS[streamId];
			...
			cmdb->bindVertexBuffer(bufferBinding, getSceneGraph().getVertexGpuMemory().getVertexBuffer(),
								   modelInf.m_vertexBufferOffsets[streamId], vertexStride, VertexStepRate::VERTEX);
		}

		// Bind index buffer
		cmdb->bindIndexBuffer(getSceneGraph().getVertexGpuMemory().getVertexBuffer(), modelInf.m_indexBufferOffset,
							  IndexType::U16);
